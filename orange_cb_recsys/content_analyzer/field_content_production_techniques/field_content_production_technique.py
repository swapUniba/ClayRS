...
class FieldContentProductionTechnique(ABC):
    ...
    def produce_content(self, field_name: str, preprocessor_list: List[InformationProcessor],
                        config) -> List[FieldRepresentation]:
        """
        ...
        EXAMPLE
            ...
            produce_content('Plot', [], config containing the raw data of the contents)
            ...
            produce_content('Plot', [NLTK()], config containing the raw data of the contents)
        ...
        Args:
            ...
            config (ContentAnalyzerConfig): config of the content analyzer who called the technique, it is useful
                since it contains the raw contents' data
...
class SingleContentTechnique(FieldContentProductionTechnique):
    ...
    def produce_content(self, field_name: str, preprocessor_list: List[InformationProcessor],
        ...
        for content_data in config.source:
...
class CollectionBasedTechnique(FieldContentProductionTechnique):
    ...
    def produce_content(self, field_name: str, preprocessor_list: List[InformationProcessor],
        ...
        self.dataset_refactor(config.source, config.id, field_name, preprocessor_list)

        representation_list: List[FieldRepresentation] = []

        # the id for each content is generated and it is used to access a specific content in the refactored collection
        for content_data in config.source:
            content_id = id_merger(content_data, config.id)
            representation_list.append(self.produce_single_repr(content_id))
    ...
    @abstractmethod
    def produce_single_repr(self, content_id: str) -> FieldRepresentation:
        """
        This method creates a single FieldRepresentation using the content id only. This content_id is used to retrieve
        the corresponding content from the dataset modified during the content creation process.

        Args:
             content_id (str): id corresponding to a content in the dataset
    ...
    @abstractmethod
    def dataset_refactor(self, information_source: RawInformationSource, id_field_names: List[str],
        ...
        Args:
            ...
            id_field_names (List[str]): fields where to find data that compound content's id
