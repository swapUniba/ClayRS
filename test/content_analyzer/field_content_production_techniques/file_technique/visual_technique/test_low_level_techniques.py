...
class TestLowLevelTechnique(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        for data in new_source:
            ...
            image = torch.from_numpy(np.array(PIL.Image.open(image_path).convert('RGB'))).moveaxis(-1, 0)
            images_list.append(image)

        cls.images_list = images_list
...
class TestHogDescriptor(TestLowLevelTechnique):
    ...
    def test_hog_descriptor(self):
        ...
        technique = SkImageHogDescriptor(flatten=True, contents_dirs=ds_path_without_field_name)
...
class TestCannyEdgeDetector(TestLowLevelTechnique):
    ...
    def test_canny_edge_detector(self):
        ...
        technique = SkImageCannyEdgeDetector(flatten=False, contents_dirs=ds_path_without_field_name)
        ...
        technique = SkImageCannyEdgeDetector(flatten=True, contents_dirs=ds_path_without_field_name)
        framework_output = technique.produce_content('imagePath', [TorchGrayscale()], source)
...
class TestColorsHist(TestLowLevelTechnique):
    ...
    def test_colors_histogram(self):
        ...
        technique = ColorsHist(contents_dirs=ds_path_without_field_name)
        framework_output = technique.produce_content('imagePath', [], source)
...
class TestColorQuantization(TestLowLevelTechnique):
    ...
    def test_color_quantization(self):
        ...
        technique = ColorQuantization(n_colors=3, flatten=False, random_state=42, contents_dirs=ds_path_without_field_name)
        framework_output = technique.produce_content('imagePath', [], source)

        # check that there are 3 outputs and all of them match the expected number of dimensions
        # in this case the shape is also explicitly checked (since 3 colors are considered it is expected that each
        # output will be similar to [20, 134, 23] where each value represents one of the image channels;
        ...
        for image in self.images_list:
            reshaped_image = (image.numpy()).reshape(image.shape[1] * image.shape[2], 3)
        ...
        # check that output is flattened when "flatten" parameter is set to true
        technique = ColorQuantization(flatten=True, contents_dirs=ds_path_without_field_name)
        framework_output = technique.produce_content('imagePath', [], source)
...
class TestSkImageSIFT(TestLowLevelTechnique):
    def test_sift(self):
        ...
        technique = SkImageSIFT(flatten=False, contents_dirs=ds_path_without_field_name)
        framework_output = technique.produce_content('imagePath', [TorchGrayscale()], source)
        ...
        # check that output is flattened when "flatten" parameter is set to true
        technique = SkImageSIFT(flatten=True, contents_dirs=ds_path_without_field_name)
        framework_output = technique.produce_content('imagePath', [TorchGrayscale()], source)
...
class TestSkImageLBP(TestLowLevelTechnique):
    ...
    def test_LBP(self):
        ...
        technique = SkImageLBP(p=8, r=2, as_image=True, contents_dirs=ds_path_without_field_name)
        framework_output = technique.produce_content('imagePath', [TorchGrayscale()], source)
        ...
        technique = SkImageLBP(p=8, r=2, as_image=False, contents_dirs=ds_path_without_field_name)
        framework_output = technique.produce_content('imagePath', [TorchGrayscale()], source)
        ...
        technique = SkImageLBP(contents_dirs=ds_path_without_field_name, as_image=True, flatten=True, p=8, r=2)
        framework_output = technique.produce_content('imagePath', [TorchGrayscale()], source)
        ...
        technique = SkImageLBP(contents_dirs=ds_path_without_field_name, as_image=False, flatten=True, p=8, r=2)
        framework_output = technique.produce_content('imagePath', [TorchGrayscale()], source)
...
class TestCustomFilterConvolution(TestLowLevelTechnique):
    def test_custom_convolution(self):
        ...
        technique = CustomFilterConvolution(weights, flatten=False, contents_dirs=ds_path_without_field_name)
        framework_output = technique.produce_content('imagePath', [TorchGrayscale()], source)
        ...
        # check that output is flattened when "flatten" parameter is set to true
        technique = CustomFilterConvolution(weights, flatten=True, contents_dirs=ds_path_without_field_name)
        framework_output = technique.produce_content('imagePath', [TorchGrayscale()], source)
