...
RS_DICT = {
    'intro': './templates_chunks/templates_rs_mini_chunks/intro_rs_and_partitioning.tex',
    'end': './templates_chunks/templates_rs_mini_chunks/end_rs.tex',
    'CentroidVector': './templates_chunks/templates_rs_mini_chunks/centroid_vector_algo.tex',
    'AmarDoubleSource': './templates_chunks/templates_rs_mini_chunks/amar_double_source_algo.tex',
    'LinearPredictor': './templates_chunks/templates_rs_mini_chunks/linear_predictor_algo.tex',
    'ClassifierRecommender': './templates_chunks/templates_rs_mini_chunks/classifier_algo.tex',
    'IndexQuery': './templates_chunks/templates_rs_mini_chunks/index_query_algo.tex',
    'NXPageRank': './templates_chunks/templates_rs_mini_chunks/recsys_template_complete.tex'
}
...
def build_final_latex_file(file_destination):
    ...
    def process_and_write_to_file2(path_dict, pros, scope1, scope2, content_container, text_container, file_path):
        # we are extracting the written part of the latex file which is retrieved from
        # the dictionary which contain for each key the path of the file that needs to be added
        content = read_file_latex(path_dict.get(pros))

        # This passage allows to specify the field in the report and prepare text
        content = content.replace('Y', scope1)
        content = content.replace('Z', scope2)
    ...
    process_and_write_to_file(REPORT_DICT, 'intro', get_current_date_string(), content_of_field, text_extract,
                              file_destination)
        ...
        for field in list_of_field:
            ...
            for process in processing_list:
                ...
                if process != 'preprocessing' and process != 'postprocessing':
                    process_and_write_to_file(CA_DICT, process, field, content_of_field, text_extract,
                                              file_destination)
                # dealing with preprocessing part
                    ...
                    if not prepro_list:
                        ...
                        process_and_write_to_file(CA_DICT, 'preprocessing', field, content_of_field, text_extract,
                                                  file_destination)
                    else:
                        # add all file latex corresponding to preprocessing techniques used
                        for prep in prepro_list:
                            process_and_write_to_file(CA_DICT, prep, field, content_of_field, text_extract,
                                                      file_destination)
                # dealing with postprocessing part
                    ...
                    if not postpro_list:
                        ...
                        process_and_write_to_file(CA_DICT, 'postprocessing', field, content_of_field, text_extract,
                                                  file_destination)
                    else:
                        # add all file latex corresponding to postprocessing techniques used
                        for postp in postpro_list:
                            process_and_write_to_file(CA_DICT, postp, field, content_of_field, text_extract,
                                                      file_destination)
    ...
    add_single_mini_template(RS_DICT, 'intro', file_destination, content_of_field, text_extract)

    # this list will be used to formatting the text depending on the algorithm used
    recsys_access_list = ['recsys', 'T', 'algorithm', 'A']
    recsys_access_methodology_list = ['recsys', 'T', 'methodology']

    # this list will get all the algorithm used during the experiment
    list_algo_type = get_keys_at_level(rs_dict, "recsys")
    print('HELLLLOOOO!!')
    print(list_algo_type)
    for cat in list_algo_type:
        # changing the access list and methodology list in order to be able to access to the right key
        index_ac_lst = recsys_access_list.index('T')
        recsys_access_list[index_ac_lst] = cat  # recsys_access_list = ['recsys', 'cat', 'algorithm', 'A']
        print(recsys_access_list)
        print(cat)
        index_metho_list = recsys_access_methodology_list.index('T')
        recsys_access_methodology_list[index_metho_list] = cat
        print(recsys_access_methodology_list)
        ...
        process_and_write_to_file2(RS_DICT, algo_key,
                                   retrieve_parameter_settings(rs_dict, recsys_access_list),
                                   retrieve_parameter_settings(rs_dict, recsys_access_methodology_list),
                                   content_of_field, text_extract, file_destination)
        # reset the list to access the dictionary of recsys yml report
        index_ac_lst = recsys_access_list.index(cat)
        recsys_access_list[index_ac_lst] = 'T'
        index_metho_list = recsys_access_methodology_list.index(cat)
        recsys_access_methodology_list[index_metho_list] = 'T'
        index_ac_lst = recsys_access_list.index(algo_key)
        recsys_access_list[index_ac_lst] = 'A'

    # closing recsys section
    add_single_mini_template(RS_DICT, 'end', file_destination, content_of_field, text_extract)
