...
def stats_relevance_tab(df, row_index, tab_title="", scientific_notation=True, round_to=4):
    """
        Using the data frame received in input this function will extract the information inside the data frame
        with the row index used to access directly to a specific row and extract the data relevant to the row,
        finally it will organise the dat into a latex table.

        Parameters:
        - df (pandas data  frame): text to be processed and changed if needed.
        - row_index (tuple): it's a tuple used to access to the data frame.
        - tab_title (str): string used as title for the table.
        - scientific_notation (bool): used to apply the function that will format the results in scientific notation.
        - round_to (int): indicate the number of cipher to round.
...
def remove_stats_from_df(dataf, removal_idx):
    """
         It will remove a columns index and its value from the original data frame.

         Parameters:
         - dataf (pandas data frame): text to be processed and changed if needed.
         - removal_idx (str): it's the name of the column of the data frame that will be removed.

         Returns:
         -  new_dataf (pandas data frame): the new data frame after processing.
...
def set_access_index(sys1, sys2, metric, type_val='pvalue'):
    """
         Create an index to access by row to a data frame.

         Parameters:
         - sys1 (str): first algorithm name.
         - sys2 (str): second algorithm name.
         - metric (str): metric to be accessed.
         - type_val (str): value of the metric.

         Returns:
         -  (row_index, column_index) (tuple): index to gain accesss to the data frame.
...
def change_system_name(df, sys_name):
    """
         This function using a dictionary will change the value of the data frame with the value of the keys.

         Parameters:
         - df (pandas data frame): data frame to be processed.
         - sys_name (str): dictionary used for substitution.

         Returns:
         - df (pandas data frame) : data frame with the values changed.
...
def from_dataframe_to_latex_table(df, col, title=""):
    """
       ...
       in particular the number of tables produced will depend on the number of columns present in the dataframe.

       Parameters:
       - df (pandas data  frame): text to be processed and changed if needed.
       - col (int): number of columns to be showed for each table created.
       - title (str): string used as title for the table. example of dict = {
   ...
   """
    ...
    # control check for structure of the table
    col = min(col, 3)
    ...
    # Iteration on the columns of the dataframe in order to produce the tables needed
    for i in range(0, len(header_cols), col):
        # Selection of the columns to be used for the current table
        selected_cols = header_cols[i:i + col]

        # Filtering data frame with the selected columns
        df_selected = df[selected_cols]

        # Add subtitle based on the columns names used
        subtitle = ', '.join(selected_cols)
        table_title = f"{title} - {subtitle}"
