...
\textbf{\lstinline[style=verbatim-text]| plot_0 |} has been represented with the following techniques:
\hfill\break
\hfill\break
% content representation on plot_0
\BLOCK{if my_dict is defined and
      my_dict['field_representations'] is defined and
      'plot_0' in my_dict['field_representations']}

\BLOCK{ set valid_keys = [] }
\begin{itemize}
    \BLOCK{ for key, value in my_dict['field_representations']['plot_0'].items() }
    ...
    \BLOCK{ for valid_key in valid_keys }
        ...
        \begin{itemize}
            \BLOCK{ for k, v in my_dict['field_representations']['plot_0'][valid_key].items() }
                \item
                \verb| \VAR{ k }: \VAR{ v }|
            \BLOCK{ endfor }
        \end{itemize}
...
\hfill\break
\hfill\break
% content_representation_field_ plot_0 ____
\BLOCK{endif}



% preprocessing on plot_0
\BLOCK{ if my_dict is defined and
      my_dict['field_representations'] is defined and
      'plot_0' in my_dict['field_representations'] and
      'preprocessing' in my_dict['field_representations']['plot_0'] and
      my_dict['field_representations']['plot_0']['preprocessing'] is defined and
      my_dict['field_representations']['plot_0']['preprocessing'] is not none }
    % dictionary 'preprocessing' not empty
\begin{itemize}
\BLOCK{ for key, value in my_dict['field_representations']['plot_0']['preprocessing'].items() }
    \item
...
\end{itemize}
\BLOCK{else}
No preprocessing techniques have been used to preprocess data \lstinline[style=verbatim-text]| plot_0 | has been represented with the following techniques:
 during the experiment.
\BLOCK{endif}
% _preprocessing on plot_0 ____
\hfill\break
\hfill\break
...
\BLOCK{if my_dict is defined and
      my_dict['field_representations'] is defined and
      'plot_0' in my_dict['field_representations'] and
      'postprocessing' in my_dict['field_representations']['plot_0'] and
      my_dict['field_representations']['plot_0']['postprocessing'] is defined and
      my_dict['field_representations']['plot_0']['postprocessing'] is not none}
    % dictionary 'preprocessing' not empty
On th field \verb| plot_0 | have been applied the following postprocessing techniques:
\begin{itemize}
\BLOCK{ for key, value in my_dict['field_representations']['plot_0']['postprocessing'].items() }
    \item
     \verb| \VAR{ key } :|
     \begin{itemize}
      \BLOCK{ for k, v in value.items() }
...
No postprocessing techniques have been used on the data \lstinline[style=verbatim-text]| plot_0 | in this experiment.
\BLOCK{endif}
\hfill\break
\hfill\break
% postprocessing_on_ plot_0 ___



% field: genres_0 representation subsection
\textbf{\lstinline[style=verbatim-text]| genres_0 |} has been represented with the following techniques:
\hfill\break
\hfill\break
% content representation on genres_0
\BLOCK{if my_dict is defined and
      my_dict['field_representations'] is defined and
      'genres_0' in my_dict['field_representations']}

\BLOCK{ set valid_keys = [] }
\begin{itemize}
    \BLOCK{ for key, value in my_dict['field_representations']['genres_0'].items() }
        \BLOCK{ if key != 'preprocessing' and key != 'postprocessing' }
            \BLOCK{ set _ = valid_keys.append(key) }
        \BLOCK{ endif }
    \BLOCK{ endfor }
    ...
    \BLOCK{ for valid_key in valid_keys }
        ...
        \verb| \VAR{ valid_key } | used as content representation technique with following parameters:
        \begin{itemize}
            \BLOCK{ for k, v in my_dict['field_representations']['genres_0'][valid_key].items() }
                \item
                \verb| \VAR{ k }: \VAR{ v }|
            \BLOCK{ endfor }
        \end{itemize}
...
\hfill\break
\hfill\break
% content_representation_field_ genres_0 ____
\BLOCK{endif}



% preprocessing on genres_0
\BLOCK{ if my_dict is defined and
      my_dict['field_representations'] is defined and
      'genres_0' in my_dict['field_representations'] and
      'preprocessing' in my_dict['field_representations']['genres_0'] and
      my_dict['field_representations']['genres_0']['preprocessing'] is defined and
      my_dict['field_representations']['genres_0']['preprocessing'] is not none }
    % dictionary 'preprocessing' not empty
\begin{itemize}
\BLOCK{ for key, value in my_dict['field_representations']['genres_0']['preprocessing'].items() }
    \item
     \verb| \VAR{ key } | used as preprocessing technique with following settings:
     \begin{itemize}
      \BLOCK{ for k, v in value.items() }
...
\end{itemize}
\BLOCK{else}
No preprocessing techniques have been used to preprocess data \lstinline[style=verbatim-text]| genres_0 | has been represented with the following techniques:
 during the experiment.
\BLOCK{endif}
% _preprocessing on genres_0 ____
\hfill\break
\hfill\break
...
\BLOCK{if my_dict is defined and
      my_dict['field_representations'] is defined and
      'genres_0' in my_dict['field_representations'] and
      'postprocessing' in my_dict['field_representations']['genres_0'] and
      my_dict['field_representations']['genres_0']['postprocessing'] is defined and
      my_dict['field_representations']['genres_0']['postprocessing'] is not none}
    % dictionary 'preprocessing' not empty
On th field \verb| genres_0 | have been applied the following postprocessing techniques:
\begin{itemize}
\BLOCK{ for key, value in my_dict['field_representations']['genres_0']['postprocessing'].items() }
    \item
     \verb| \VAR{ key } :|
     \begin{itemize}
      \BLOCK{ for k, v in value.items() }
...
\BLOCK{else}
% This block is rendered if no post processing technique has been applied
No postprocessing techniques have been used on the data \lstinline[style=verbatim-text]| genres_0 | in this experiment.
\BLOCK{endif}
\hfill\break
\hfill\break
% postprocessing_on_ genres_0 ___



% field: genres_1 representation subsection
\textbf{\lstinline[style=verbatim-text]| genres_1 |} has been represented with the following techniques:
\hfill\break
\hfill\break
% content representation on genres_1
\BLOCK{if my_dict is defined and
      my_dict['field_representations'] is defined and
      'genres_1' in my_dict['field_representations']}

\BLOCK{ set valid_keys = [] }
\begin{itemize}
    \BLOCK{ for key, value in my_dict['field_representations']['genres_1'].items() }
        \BLOCK{ if key != 'preprocessing' and key != 'postprocessing' }
            \BLOCK{ set _ = valid_keys.append(key) }
        \BLOCK{ endif }
    \BLOCK{ endfor }
    ...
    \BLOCK{ for valid_key in valid_keys }
        ...
        \verb| \VAR{ valid_key } | used as content representation technique with following parameters:
        \begin{itemize}
            \BLOCK{ for k, v in my_dict['field_representations']['genres_1'][valid_key].items() }
                \item
                \verb| \VAR{ k }: \VAR{ v }|
            \BLOCK{ endfor }
        \end{itemize}
...
\hfill\break
\hfill\break
% content_representation_field_ genres_1 ____
\BLOCK{endif}



% preprocessing on genres_1
\BLOCK{ if my_dict is defined and
      my_dict['field_representations'] is defined and
      'genres_1' in my_dict['field_representations'] and
      'preprocessing' in my_dict['field_representations']['genres_1'] and
      my_dict['field_representations']['genres_1']['preprocessing'] is defined and
      my_dict['field_representations']['genres_1']['preprocessing'] is not none }
    % dictionary 'preprocessing' not empty
\begin{itemize}
\BLOCK{ for key, value in my_dict['field_representations']['genres_1']['preprocessing'].items() }
    \item
     \verb| \VAR{ key } | used as preprocessing technique with following settings:
     \begin{itemize}
      \BLOCK{ for k, v in value.items() }
...
\end{itemize}
\BLOCK{else}
No preprocessing techniques have been used to preprocess data \lstinline[style=verbatim-text]| genres_1 | has been represented with the following techniques:
 during the experiment.
\BLOCK{endif}
% _preprocessing on genres_1 ____
\hfill\break
\hfill\break
...
\BLOCK{if my_dict is defined and
      my_dict['field_representations'] is defined and
      'genres_1' in my_dict['field_representations'] and
      'postprocessing' in my_dict['field_representations']['genres_1'] and
      my_dict['field_representations']['genres_1']['postprocessing'] is defined and
      my_dict['field_representations']['genres_1']['postprocessing'] is not none}
    % dictionary 'preprocessing' not empty
On th field \verb| genres_1 | have been applied the following postprocessing techniques:
\begin{itemize}
\BLOCK{ for key, value in my_dict['field_representations']['genres_1']['postprocessing'].items() }
    \item
     \verb| \VAR{ key } :|
     \begin{itemize}
      \BLOCK{ for k, v in value.items() }
...
\BLOCK{else}
% This block is rendered if no post processing technique has been applied
No postprocessing techniques have been used on the data \lstinline[style=verbatim-text]| genres_1 | in this experiment.
\BLOCK{endif}
\hfill\break
\hfill\break
% postprocessing_on_ genres_1 ___
...
% ------------------------------- RESULT OF PERFORMACE OF THE SYSTEM ON THE FOLD sys - fold1 -------------------------------------
\subsection{Results for sys - fold1}\label{subsec:sys - fold1}
In this section we show the results obtained on the fold: \textbf{ sys - fold1 }, the metrics used during the experiment will be
grouped into the following table, that represent the results of the evaluation conducted~\ref{tab:results_table_sys - fold1}
...
\hfill\break
\hfill\break
% end table of performance on the fold sys - fold1 ___
\BLOCK{endif}



\BLOCK{if my_dict['sys_results'] is defined}
% ------------------------------- RESULT OF PERFORMACE OF THE SYSTEM ON THE FOLD sys - mean -------------------------------------
\subsection{Results for sys - mean}\label{subsec:sys - mean}
In this section we show the results obtained on the fold: \textbf{ sys - mean }, the metrics used during the experiment will be
grouped into the following table, that represent the results of the evaluation conducted~\ref{tab:results_table_sys - mean}
...
\hfill\break
\hfill\break
% end table of performance on the fold sys - mean ___
\BLOCK{endif}
